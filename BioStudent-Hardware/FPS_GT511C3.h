...
class Command_Packet
{
	...
	private:
		...
		wordnew _CalculateChecksum();						// Checksum is calculated using byte addition
		byte GetHighByte(wordnew w);
		byte GetLowByte(wordnew w);
};
...
class Response_Packet
{
	...
	private:
		...
		wordnew CalculateChecksum(byte* buffer, int length);
		byte GetHighByte(wordnew w);
		byte GetLowByte(wordnew w);
};
