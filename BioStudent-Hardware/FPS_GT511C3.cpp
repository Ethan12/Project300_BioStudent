...
byte* Command_Packet::GetPacketBytes()
{
	...
	wordnew checksum = _CalculateChecksum();
...
// Returns the high byte from a wordnew
byte Command_Packet::GetHighByte(wordnew w)
{
	return (byte)(w>>8)&0x00FF;
}

// Returns the low byte from a wordnew
byte Command_Packet::GetLowByte(wordnew w)
{
	return (byte)w&0x00FF;
}

wordnew Command_Packet::_CalculateChecksum()
{
	wordnew w = 0;
...
Response_Packet::Response_Packet(byte* buffer, bool UseSerialDebug)
{
	...
	wordnew checksum = CalculateChecksum(buffer, 10);
...
wordnew Response_Packet::CalculateChecksum(byte* buffer, int length)
{
	wordnew checksum = 0;
...
// Returns the high byte from a wordnew
byte Response_Packet::GetHighByte(wordnew w)
{
	return (byte)(w>>8)&0x00FF;
}

// Returns the low byte from a wordnew
byte Response_Packet::GetLowByte(wordnew w)
{
	return (byte)w&0x00FF;
}
